...
<head>
    ...
    <title>@yield('title', 'SmartStudy Portal')</title>
    ...
    <aside class="hidden lg:flex flex-col w-72 sidebar-gradient text-white sticky top-0 h-screen overflow-y-auto">
        <div class="p-8 flex-grow">
            <div class="flex items-center space-x-3 mb-12">
                ...
                <span class="text-xl font-bold tracking-tight">SmartStudy</span>
            </div>
    ...
    <div x-data="{ mobileMenu: false }" class="flex-grow min-h-screen flex flex-col w-full">
                ...
                <div class="flex items-center justify-between mb-12">
                    ...
                    <div class="mb-12 text-center">
                        <div class="relative inline-block mb-4">
                            <a href="{{ route('profile.index') }}" class="block">
                                @if($user && $user->profile_photo_url && !str_contains($user->profile_photo_url, 'ui-avatars'))
                                    <img src="{{ $user->profile_photo_url }}" alt="Profile" class="w-20 h-20 rounded-full border-4 border-white/20 object-cover">
                                @else
                        ...
                        <h3 class="font-bold text-base mb-1">{{ $user->name ?? 'Student' }}</h3>
                        <p class="text-brand-300 text-xs font-medium uppercase tracking-widest">Siswa Pelajar</p>
                    </div>
        ...
        <header class="bg-white/80 backdrop-blur-md sticky top-0 z-40 border-b border-slate-200 lg:px-12 px-6 py-4 flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                <h1 class="text-xl font-extrabold text-slate-900 tracking-tight lg:block hidden">@yield('header', 'SmartStudy Portal')</h1>
        ...
        <footer class="px-12 py-6 text-center text-slate-400 text-xs border-t border-slate-200 mt-auto">
            &copy; {{ date('Y') }} SmartStudy. Empowering Knowledge.
        </footer>
    </div>
